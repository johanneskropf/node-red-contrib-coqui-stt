...
module.exports = function(RED) {
    ...
    function coquiStreamNode(config) {
        ...
        if (!node.errorStop) {
            try {
                node.decoder = new stt.Model(node.modelPath);
                        ...
                        case "stop":
                            if (node.streamDecoder){
                                try {
                                    ...
                                    stt.FreeStream(node.streamDecoder);
                                    node.streamDecoder = null;
                                } catch (error) {
                                    node.error(`couldnt clean up coqui stream: ${error}`)
                                }
                            } else {
        ...
        node.on("close",function() {
            ...
            if (node.decoder) {
                try {
                    stt.FreeModel(node.decoder);
                    node.decoder = null;
                } catch (error) {
                    node.error(`couldnt clean up coqui: ${error}`)
                }
            }
        });
        
    }
    RED.nodes.registerType("coqui-stream",coquiStreamNode);
}